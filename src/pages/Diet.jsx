...
const Diet = () => {
    ...
    const fetchSegments = async () => {
        try {
            ...
            const response = await fetch(
                "https://santhai.kooturavu.tn.gov.in/api/v1//segment",
                {
                    headers: {
    ...
    const fetchEaterTypes = async () => {
        try {
            ...
            const response = await fetch(
                "https://santhai.kooturavu.tn.gov.in/api/v1//master/eater-type",
                {
    ...
    const fetchItems = async () => {
        try {
            ...
            const response = await fetch(
                "https://santhai.kooturavu.tn.gov.in/api/v1//segment/items",
                {
                    headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
                }
            );
    ...
    const handleCreateSegment = async () => {
        try {
            ...
            for (const payload of payloads) {
                await fetch("https://santhai.kooturavu.tn.gov.in/api/v1//segment", {
                    method: "POST",
                    headers: {
                        Authorization: `Bearer ${token}`,
                        "Content-Type": "application/json",
  ...
  setTimeout(() => {
    ...
    const handleDeletePlan = async (segmentId) => {
        try {
            ...
            const response = await fetch(
                `https://santhai.kooturavu.tn.gov.in/api/v1//segment/${segmentId}/plan`,
                {
                    method: "POST",
                    headers: {
                        Authorization: `Bearer ${token}`,
...
if (mealData.items || Object.values(mealData).some(v => v?.items)) {
    ...
    const fetchSegmentDetail = async (segmentId) => {
        try {
            ...
            const response = await fetch(
                "https://santhai.kooturavu.tn.gov.in/api/v1//segment/detail",
                {
                    method: "POST",
                    headers: {
                        Authorization: `Bearer ${token}`,
  ...
  <div className="flex items-center gap-2">
    ...
    <button className="text-blue-600 hover:text-blue-800">
      <i className="fas fa-arrow-right"></i>
                                    ...
                                    {/* Inline Plan Details - Expandable */}
...
if (planData.month) {
    ...
    Object.entries(planData.month).forEach(([mealType, mealObj]) => {
        const items = [];

        // Handle both possible structures (with or without category nesting)
        if (mealObj.items) {
            // Direct month plan (like { items: {id: qty} })
            Object.entries(mealObj.items).forEach(([itemId, qty]) => {
                const foundItem = allItems.find(item => item.id == itemId);
                items.push({
                    category: "food",
                    item: foundItem ? foundItem.id : itemId,
                    qty,
                    unit: foundItem?.resident || "Grams",
                });
            });
        } else {
            // Fallback to nested category structure
            Object.entries(mealObj || {}).forEach(([category, categoryItems]) => {
                Object.entries(categoryItems || {}).forEach(([itemName, itemData]) => {
                    const foundItem = allItems.find(item => item.name === itemName);
                    items.push({
        ...
        monthPlan.meals.push({ type: mealType, items });
    });
            ...
            {showPopup && (
                <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
                    <div className="bg-white p-6 rounded-lg shadow-lg w-96">
                        <h3 className="text-lg font-bold mb-4">Eater Diet Type </h3>
