...
import { Eye, FileText, Upload, FileCheck, MoreVertical } from 'lucide-react';
...
const handleBilling = async (order) => {
  ...
  try {
    ...
    const response = await fetch(
      `https://santhai.kooturavu.tn.gov.in/api/v1/bill/list/${id}`,
      {
        method: "GET",
        headers: {
  ...
  const fetchOrders = async () => {
    try {
      const token = localStorage.getItem('authToken');
      const response = await fetch(`https://santhai.kooturavu.tn.gov.in/api/v1//indent/approved/list`, {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${token}`,
...
const handleConfirmGrnSubmission = async () => {
  ...
  const itemsPayload = selectedOrderItems.map((item, index) => {
    ...
    if (isReceived) {
      if (!isReturn) payload.qty = Number(receivedQuantities[index] || 0);
...
const handleConfirmCancelGrn = async (remarks) => {
  try {
    const token = localStorage.getItem('authToken');
    const response = await fetch(`${API_BASE_URL}/notifications/indent/grn-del`, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${token}`,
        'Content-Type': 'application/json',
      ...
      body: JSON.stringify({ 
        indent: cancelGrnOrderId,
        remarks: remarks 
      }),
    });

    if (response.ok) {
      setSuccessMessage('GRN cancel request submitted successfully!');
      fetchOrders();
    } else {
      const errorResult = await response.json();
      console.error('Cancel request failed:', errorResult);
      setSuccessMessage('Cancel request failed. Please try again.');
    }
  } catch (error) {
    console.error('Error during cancel request:', error);
    setSuccessMessage('An error occurred while requesting cancellation.');
  } finally {
...
${index === currentOrders.length - 1 ? "bottom-full mb-2" : "mt-2"}`}
        >
          <ul className="py-1 text-sm text-gray-700">
            ...
            {!uploadedFiles[order.id] && JSON.parse(localStorage.getItem('user'))?.role?.toLowerCase() === 'supply' && (
              <li>
                <button
...
<div className="flex justify-between items-center mt-4">
                    ...
                    <tr>
                      ...
                      {(selectedOrderStatus === 'Dispatch' || selectedOrderStatus === 'Received') && (
                        <>
                          ...
                          <th className="border border-gray-300 px-2 py-2 text-center w-[100px]">Packs</th>
                        </>
                      )}
                                      ...
                                      return (
                                        <>
                                          <input
                                            ...
                                            step={packSize}
                                            min={0}
                                            max={item.disp_qty} // ✅ block values greater than dispatched qty
                                            ...
                                            onChange={(e) => {
                                              ...
                                              // 🚫 Clamp to dispatched qty
                                              if (value > item.disp_qty) {
                                                value = item.disp_qty;
                                              }

                                              if (packSize > 0.1) {
                                                // ✅ enforce multiples of packSize
                                                const remainder = value % packSize;
                                                if (remainder !== 0) {
                                                  value = Math.round(value / packSize) * packSize;
                                                }
                                              } else {
                                                // ✅ fallback: allow 0.1 step, round to 1 decimal
                                          ...
                                          <input
                                            type="text"
                                            className="border border-gray-300 rounded px-1 py-2 text-sm flex-1"
                                            value={remarks[index] || ''}
                                            onChange={(e) =>
                                              setRemarks((prev) => ({
                                                ...prev,
  ...
  className={`px-6 py-3 rounded-lg text-white transition ${
    isStatusSelectedForAll && !submitting
...
{/* <RequestPopup
  ...
  loading={loadingCancelItems}
    userRole={userRole} // "rcs-store" or "rcs-supply"
/> */}
{billingPopup && (
