...
const StockNewNotifications = () => {
  ...
  const fetchStockMap = async (token) => {
    try {
      const response = await fetch(
        "https://santhai.kooturavu.tn.gov.in/api/v1//item/list/stock",
        {
          headers: {
            Authorization: `Bearer ${token}`,
            "Content-Type": "application/json",
  ...
  const handleApprove = async (notificationId, notification) => {
    try {
      ...
      const response = await fetch(
        `https://santhai.kooturavu.tn.gov.in/api/v1//item/stock/update/${notificationId}/approve`,
        {
          method: 'PUT',
          headers: {
            Authorization: `Bearer ${token}`,
 ...
 if (!result.error && response.ok) {
  ...
  const handleDecline = async (notificationId) => {
    try {
      ...
      const response = await fetch(
        `https://santhai.kooturavu.tn.gov.in/api/v1//item/stock/update/${notificationId}/reject`,
        {
          method: 'PUT',
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  {itemMap[id].unit}
          ...
          <td className="border px-3 py-2 text-center font-semibold">
            {(() => {
              ...
              if (value?.type === "issued") return current - val;
              if (value?.type === "received") return current + val;
              if (value?.type === "opening") return val;
              return "—";
            })()}
          </td>
        </tr>
