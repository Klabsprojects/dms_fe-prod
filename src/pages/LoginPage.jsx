  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Extract and store range if available
try {
  const rangeData = data?.user?.detail?.range 
    || JSON.parse(data?.others || '{}')?.range;

  if (rangeData) {
    localStorage.setItem('rangeData', JSON.stringify(rangeData));
    console.log('✅ Range data stored in localStorage:', rangeData);
  } else {
    console.warn('⚠️ No range data found in login response');
  }
} catch (err) {
  console.error('❌ Error parsing/storing range data:', err);
}
        ...
        if (role === 'ind-apr') {
         ...
         else if (['rcs-admin','indent'].includes(role)) {
          window.location.replace('/dashboard');
        }
         ...
         else if (['department'].includes(role)) {
          window.location.replace('/dashboard');
        }
...
Department of Cooperation
  ...
  <a
    href="https://prison-mart.onlinetn.info/"
