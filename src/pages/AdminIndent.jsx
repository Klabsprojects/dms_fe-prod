...
useEffect(() => {
  ...
  try {
    ...
    const response = await fetch(
      `https://santhai.kooturavu.tn.gov.in/api/v1/bill/list/${orderId}`,
      {
        method: "GET",
        headers: {
          Authorization: `Bearer ${token}`,
 ...
 if (storeFilter && storeFilter !== "all") {
  ...
  const handleInvoiceUpload = async (orderId, file) => {
    ...
    try {
      ...
      const response = await fetch(`https://santhai.kooturavu.tn.gov.in/api/v1//indent/${orderId}/invoice`, {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${token}`,
        },
  ...
  const handlePaymentTransfer = async (orderId) => {
    try {
      ...
      const response = await fetch(`https://santhai.kooturavu.tn.gov.in/api/v1//indent/${orderId}`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
  ...
  const handleConfirmPcDelete = async (remarks) => {
    ...
    try {
      const token = localStorage.getItem("authToken");
      const response = await fetch(
        `https://santhai.kooturavu.tn.gov.in/api/v1/indent/${selectedPcData.orderId}/pc-del`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${token}`,
...
const handleViewInvoices = async (orderId) => {
  ...
  const handleConfirmInvDelete = async (remarks) => {
    ...
    try {
      ...
      const response = await fetch(
        "https://santhai.kooturavu.tn.gov.in/api/v1//notifications/indent/inv-del",
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
...
order.depatment !== departmentFilter
