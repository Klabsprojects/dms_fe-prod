...
const handleApprove = async (orderId) => {
  ...
  try {
    ...
    const response = await fetch(
      `https://santhai.kooturavu.tn.gov.in/api/v1/indent/${orderId}/approve`,
      {
        method: "PUT",
        headers: {
          "Authorization": `Bearer ${token}`,
  ...
  const handleReject = async () => {
    ...
    try {
      const token = localStorage.getItem('authToken');
      const response = await fetch(`${API_BASE_URL}/indent/${selectedOrderId}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
  ...
  const handleConfirmCancel = async (remarks) => {
    ...
    try {
      const token = localStorage.getItem("authToken");
      const response = await fetch(
        `${API_BASE_URL}/indent/${selectedIndentForCancel.id}/cancel`,
        {
          method: "PUT",
          headers: {
            "Authorization": `Bearer ${token}`,
      ...
      if (response.ok && !result.error) {
        setActionSuccess("Cancel request submitted successfully!");
        // Refresh list
        ...
        setError(result.message || "Failed to submit cancel request");
      }
    } catch (err) {
      setError("Network error while submitting cancel request");
    }
  };
  ...
  const fetchPendingIndentItems = async (indentId) => {
    try {
      ...
      const response = await fetch(
        `https://santhai.kooturavu.tn.gov.in/api/v1/indent/${indentId}/items`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
            "Content-Type": "application/json",
