...
const IndentCreation = () => {
  ...
  const fetchMedicalSegments = async () => {
    try {
      const token = localStorage.getItem('authToken');
      const response = await fetch(`https://santhai.kooturavu.tn.gov.in/api/v1//master/eater-type`, {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
  ...
  useEffect(() => {
    if (indentType === 'medical') {
      ...
      const eaterApi = fetch('https://santhai.kooturavu.tn.gov.in/api/v1//master/eater-type', {
        headers: { 'Authorization': `Bearer ${token}` }
      }).then(res => res.json());

      const segmentApi = fetch('https://santhai.kooturavu.tn.gov.in/api/v1//segment', {
        headers: { 'Authorization': `Bearer ${token}` }
      }).then(res => res.json());
  ...
  const fetchStoreDetails = async () => {
    try {
      ...
      const response = await fetch(`https://santhai.kooturavu.tn.gov.in/api/v1//user/store/${storeName}`, {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
  ...
  useEffect(() => {
    const fetchSegments = async () => {
      try {
        const token = localStorage.getItem("authToken");
        const response = await fetch("https://santhai.kooturavu.tn.gov.in/api/v1//segment", {
          headers: {
            Authorization: `Bearer ${token}`,
            "Content-Type": "application/json",
          },
  ...
  return (
    <div className="w-full max-w-7xl mx-auto p-2 sm:p-4 lg:p-6 bg-gray-50 rounded-lg h-screen flex flex-col">
      <div className="flex flex-col lg:flex-row gap-4 lg:gap-6 items-stretch h-full">
        <div className={`w-full ${(!showIndentDetails && !showExpandedView) ? 'lg:w-3/4' : 'lg:w-full'}`} ref={leftRef}>
                          ...
                          <div className="flex justify-between items-start">
                            <div className="flex-1">
                              <div className="flex justify-between items-center mb-1">
                                ...
                                <span
                                  className={`px-2 py-1 rounded text-xs font-semibold ${indent.status?.toLowerCase() === 'approved'
